          ...
          <ul className="flex flex-col gap-2">
            {records.map((r) => (
              <li key={r.id} className="rounded-md border border-line">
                <Link href={`/admin/records/${r.id}`} className="flex flex-wrap items-center justify-between gap-x-4 gap-y-1 p-3 hover:bg-soft">
                  <span className="flex flex-col">
                    <span className="font-medium">{r.subject_name}</span>
                    <span className="text-sm text-muted">{r.owner_email}</span>
                  </span>
      ...
      <section className="flex flex-col gap-3">
        ...
        <ul className="flex flex-col gap-2">
          {accounts.map((a) => (
            <li
              key={a.id}
              className="flex flex-wrap items-center justify-between gap-x-4 gap-y-2 rounded-md border border-line p-3"
            >
              <div className="flex flex-col">
                <span className="font-medium">
                  {a.name}
                  {a.role === "admin" && (
                    <span className="ml-2 rounded-full bg-soft px-2 py-0.5 text-xs font-normal">admin</span>
                  )}
                </span>
                <span className="text-sm text-muted">
      ...
      <section className="flex flex-col gap-3">
        ...
        {counts.length ? (
          <ul className="flex flex-wrap gap-2">
            {counts.map((c) => (
              <li key={c.error_type} className="rounded-md border border-line px-3 py-1.5 text-sm">
                <span className="font-medium">{c.error_type}</span>{" "}
                <span className="tabular-nums text-muted">×{c.n}</span>{" "}
                <span className="text-faint">· last {when(c.last_at)}</span>
              </li>
            ))}
          </ul>
        ) : (
          <p className="text-sm text-muted">Nothing has failed in the last week.</p>
        )}
      </section>
      ...
      <section className="flex flex-col gap-3">
        ...
        {errors.length ? (
          <ol className="flex flex-col gap-2">
            {errors.map((e) => (
              <li key={e.id} className="flex flex-col gap-1 rounded-md border border-line p-3 text-sm">
                <div className="flex flex-wrap items-baseline gap-x-3 gap-y-1">
                  <span className="font-medium">{e.error_type}</span>
                  <span className="text-muted">{e.context}</span>
                  {e.status_code !== null && <span className="text-muted">HTTP {e.status_code}</span>}
                  {e.model && <span className="text-muted">{e.model}</span>}
                  {e.duration_ms !== null && (
                    <span className="tabular-nums text-muted">{(e.duration_ms / 1000).toFixed(1)}s</span>
                  )}
                  <span className="ml-auto text-faint">{when(e.created_at)}</span>
                </div>
                <p className="break-words font-mono text-xs text-muted">{e.message}</p>
                {e.record_id && <p className="font-mono text-xs text-faint">record {e.record_id}</p>}
              </li>
            ))}
          </ol>
        ) : (
